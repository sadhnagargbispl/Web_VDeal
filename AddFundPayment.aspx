<%@ Page Title="" Language="C#" MasterPageFile="~/SiteMaster.master" AutoEventWireup="true" CodeFile="AddFundPayment.aspx.cs" Inherits="AddFundPayment" %>
                                ...
                                <div class="form-group">
                                    ...
                                    <asp:RadioButtonList ID="RbtCheckOptin" runat="server" RepeatDirection="Horizontal" CssClass="form-control"
                                       ...
                                       <%-- <asp:ListItem Text="&nbsp;&nbsp;SUMMIT-SOL TOKEN" Value="M" Selected="True" style="padding: 10px 10px;" OnClientClick="callJavaScriptFunction(); return true;"></asp:ListItem>--%>
                                        <asp:ListItem Text="&nbsp;&nbsp;USDT-BEP20" Value="B" Selected="True" style="padding: 10px 10px;"></asp:ListItem>
                                    </asp:RadioButtonList>
                                </div>
...
in your account.</b>
                                ...
                                <div runat="server" id="DivMMIT" visible="true">
                                    ...
                                    <asp:Button ID="BtnconfirmBtotton" runat="server" Text="Confirmed" class="btn btn-primary buttoncss text-white w-100" OnClick="BtnconfirmBtotton_Click" />
                                </div>
                        ...
                        <div runat="server" id="DivQr" visible="false">
                            ...
                            <div class="d-flex align-content-center  justify-content-center  gx-2 mt-2">
                                <b>
                                    <p style="color: Black; font-size: 0.8em; font-weight: bold;">
                                        Transfer Token : &nbsp;  
                                        <span id="TokenNoOF" runat="server" style="color: Black; font-weight: bold;"></span>
                                    </p>

                                </b>
                                <a class="float-right" href="#" onclick="return copyText1();" style="margin-left: 10px;">
                            ...
                            <div class="d-block mt-3" align="center">
                                <p style="font-size: 1em; color: red">
                                    <b class="blink" style="opacity: 0.618249;">waiting for user payment initiation<br />
                                        (The Average Time For Payment Confirmation Upon Success Is Approximately 30 Minutes.)</b>
                                </p>
    ...
    <script type="text/javascript">
        function callJavaScriptFunction() {
            ...
            var totalAmount = $("#ContentPlaceHolder1_TokenNoOF").text(); // Token amount (ID number)
            var reqamount = $("#ContentPlaceHolder1_amountreq").val(); // Requested amount

            // Encode URI components to handle special characters
            var ajax_url = 'QrCodehenr.ashx?payment_id=' + ID + '&idno=' + idno + '&tokenamount=' + totalAmount + '&reqamount=' + reqamount; // Generate ajax URL

            console.log(ajax_url); // Log the URL for debugging
        ...
        function status_loop(is_paid, ajax_url) {
            ...
            var ID = $("#ContentPlaceHolder1_hdnPaymentId").val(); // Payment ID
            var idno = $("#ContentPlaceHolder1_hdnPayID").val(); // ID number
            var totalAmount = $("#ContentPlaceHolder1_TokenNoOF").text(); // ID number
            var reqamount = $("#ContentPlaceHolder1_amountreq").val(); // ID number
            if (is_paid) return;

            $.ajax({
                type: "GET",
                url: "QrCodehenr.ashx?payment_id=" + ID + "&idno=" + idno + "&tokenamount=" + totalAmount + "&reqamount=" + reqamount + "",
                datatype: "json",
                ...
                success: function (data) {
                    ...
                    if (d.Response == "Success") {
                        $("#dtext").html(d.message); // Display the success message
                        alert("Payment Successfully Added in Wallet!");
                        is_paid = true; // Set the payment status
                        window.location.href = s; // Redirect to another page
                    }
                    else if (d.Response == "TokenRe") {
                        $("#dtext").html(d.message); // Display the message for remaining token deposit
                        alert("You Have Deposited " + d.balance + ". Please Deposit Remaining Token: " + d.Rembalance);
                    }
                    else {
                        $("#dtext").html(d.message); // Ensure the correct response is shown if it doesn't match the above cases
                    }
        ...
        setInterval(callJavaScriptFunction, 5000);
    </script>
