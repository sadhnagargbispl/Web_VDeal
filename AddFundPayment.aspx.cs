...
public partial class AddFundPayment : System.Web.UI.Page
{
    ...
    protected void TxtReqAmount_TextChanged(object sender, EventArgs e)
    {
        ...
        Session["REqAmount"] = TotalRate;
        amountreq.Value = TxtReqAmount.Text;
        double Bonus = 0;
        ...
        //    ResultAmount = TotalRate - Bonus;
        //}
        TxtINRValue.Text = (Convert.ToDouble(TotalRate) / 88).ToString(); //Convert In Usdt
        decimal inrValue;
        decimal rate;
        ...
        if (decimal.TryParse(TxtINRValue.Text, out inrValue) && decimal.TryParse(HdnRate.Value, out rate))
        {
            ...
            if (rate != 0)
            {
                tokenAmount = Convert.ToDecimal(inrValue) / Convert.ToDecimal(HdnRate.Value); //Convert In Inr 
    ...
    protected void BtnconfirmBtotton_Click(object sender, EventArgs e)
    {
        ...
        if (double.TryParse(TxtTokenAmount.Text, out tokenAmount) && tokenAmount > 0)
        {
            ...
            decimal tokenAmountS =Convert.ToDecimal(TxtTokenAmount.Text.ToString());
            TokenNoOF.InnerText = TxtTokenAmount.Text.ToString();
            GenerateQrCodeSummitSOl(Session["Formno"].ToString(), tokenAmountS);
    ...
    private void GenerateQrCodeSummitSOl(string Formno, decimal TokenAMount)
    {
        HdnRate.Value = MMIDUSDT();
        string requestBody;
        ...
        try
        {
            ...
            var content = new
            {
                userId = Session["idno"],
                chain = "SOL",
                amount = Convert.ToDecimal(TokenAMount) // Explicitly set to SUMMIT-SOL
            ...
            using (var streamReader = new StreamReader(response.GetResponseStream()))
            {
                ...
                //SpnAddress.InnerText = "6MYHqNrdarQNPKaToMjpW8rPwyeoS9EhAtmY7NiGesGY";
                QRCodeGeneratorHelper qrHelper = new QRCodeGeneratorHelper();
                ImgQrCode.Src = qrHelper.GenerateQRCodeDataURL(SpnAddress.InnerText, Convert.ToDecimal(TokenAMount));
                fromidno.Value = Formno.ToString();
                HdnFormno.Value = Formno.ToString();
                string strs = "";
            ...
            if (i > 0)
            {
                ...
                DivMMIT.Visible = false;
                hdnPaymentId.Value = SpnAddress.InnerText;
               ScriptManager.RegisterStartupScript(this, this.GetType(), "callJavaScriptFunction", "setTimeout(function() { callJavaScriptFunction(); }, 1000);", true);
        ...
        catch (WebException ex)
        {
            ...
            string errorQry = "";
            errorQry = "insert Into TrnLogData(ErrorText, LogDate, Url, WalletAddress,ForType,postdata)";
            errorQry += "values('" + ex.Message + "', getdate(), '" + CompleteUrl + "', '" + SpnAddress.InnerText + "','A','" + errorResponse.ToString() + "')";
            int x1 = SqlHelper.ExecuteNonQuery(ConfigurationManager.ConnectionStrings["constr"].ConnectionString, CommandType.Text, errorQry);
...
public class QRCodeGeneratorHelper
{
    ...
    public string GenerateQRCodeDataURL(string address, decimal TokenAMount)
    {
        ...
        try
        {
            ...
            string tokenMintAddress = "A3vBwL3Pd6nkBqVo87D6TLWYG4ekWuDeqCoLFeyvRMcb";
            // string qrCodeContent = $"solana:{address}";
            string qrCodeContent = $"solana:{address}?spl-token={tokenMintAddress}";
